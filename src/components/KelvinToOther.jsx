...
const KelvinToOther = () => {
	...
	const conversion = () => {
		...
		} else if (codeSelect === "KtoF") {
			setConversionResult(fahrenheit(first));
		} else if (codeSelect === "KtoR") {
	...
	const codeFocus = codeSelect === "KtoC" || codeSelect === "KtoF" || codeSelect === "KtoR";
					...
					{inputValue && inputValue !== "0" && (
						<div className="flex w-5/6 flex-col items-start gap-7 tracking-wide">
							...
							<p className="text-2xl font-bold text-primary/60">
								{codeSelect === "KtoC" && <>C = K − 273.15</>} {codeSelect === "KtoF" && <>F = (K − 273.15) * 9 / 5 + 32</>} {codeSelect === "KtoR" && <>R = (K - 273.15) * 4 / 5</>}
							</p>
							...
							<p className="text-2xl font-bold text-primary/60">
								...
								{codeSelect === "KtoF" && (
									<>
										F = ({inputValue} − 273.15) * 9 / 5 + 32 <span className="block">= ({inputValue - 273.15}) * 9 / 5 + 32</span>
										<span className="block">
											= {conversionResult}{" "}
											<span className="font-medium text-light/50">
												{convertTo !== "Kelvin" && "derajat"} {convertTo}
								...
								{codeSelect === "KtoR" && (
									<>
										R = ({inputValue} - 273.15) * 4 / 5 <span className="block">= ({inputValue - 273.15}) * 4 / 5</span>
										<span className="block">
											= {conversionResult}{" "}
											<span className="font-medium text-light/50">
												{convertTo !== "Kelvin" && "derajat"} {convertTo}
