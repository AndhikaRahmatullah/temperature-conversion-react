...
const RéaumurToOther = () => {
	...
	const conversion = () => {
		...
		} else if (codeSelect === "RtoF") {
			setConversionResult(fahrenheit(first));
		} else if (codeSelect === "RtoK") {
	...
	const codeFocus = codeSelect === "RtoC" || codeSelect === "RtoF" || codeSelect === "RtoK";
					...
					{inputValue && inputValue !== "0" && (
						<div className="flex w-5/6 flex-col items-start gap-7 tracking-wide">
							...
							<p className="text-2xl font-bold text-primary/60">
								{codeSelect === "RtoC" && <>C = R / 0.8</>} {codeSelect === "RtoF" && <>F = (R * 9) / 4 + 32</>} {codeSelect === "RtoK" && <>K =(R * 5) / 4 + 273.15</>}
							</p>
							...
							<p className="text-2xl font-bold text-primary/60">
								...
								{codeSelect === "RtoF" && (
									<>
										F = ({inputValue} * 9) / 4 + 32 <span className="block">= ({inputValue * 9}) / 4 + 32</span>
										<span className="block">
											= {conversionResult}{" "}
											<span className="font-medium text-light/50">
												{convertTo !== "Kelvin" && "derajat"} {convertTo}
								...
								{codeSelect === "RtoK" && (
									<>
										K = ({inputValue} * 5) / 4 + 273.15 <span className="block">= ({inputValue * 5}) / 4 + 273.15</span>
										<span className="block">
											= {conversionResult}{" "}
											<span className="font-medium text-light/50">
												{convertTo !== "Kelvin" && "derajat"} {convertTo}
